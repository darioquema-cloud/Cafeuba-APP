...
import{mesAnioDe,mesAnioTrillaDe}from"./dates";
...
export const calcCosto=(lote,costos,lotes)=>{
  ...
  if(lote.origen_lote!=="trilla_directa"){
    const costosCBMes=(costos||[]).filter(c=>c.centro==="Central de Beneficio"&&c.mesAnio===lote.mesAnio).reduce((s,c)=>s+c.valor,0);
    const kgPergaminoMes=(lotes||[lote]).filter(l=>l.mesAnio===lote.mesAnio&&l.kg_producto>0&&l.origen_lote!=="trilla_directa"&&l.tipo!=="Manual"&&l.origen_lote!=="carga_directa").reduce((s,l)=>s+l.kg_producto,0);
    c_val=kgPergaminoMes>0?costosCBMes/kgPergaminoMes:0;
  }
  return{totalCereza,totalIns,a,b,c:c_val,total:a+b+c_val};
};
export const calcCostoTri=(mesAnio,costos,lotes)=>{
  const costosTri=(costos||[]).filter(c=>c.centro==="Trilladora"&&c.mesAnio===mesAnio).reduce((s,c)=>s+c.valor,0);
  const kgEx=lotes.filter(l=>mesAnioTrillaDe(l)===mesAnio&&l.trilla?.kg_excelso>0).reduce((s,l)=>s+(l.trilla.kg_excelso||0),0);
...
export const costoKgExDe=(l,costos,lotes)=>{
  ...
  const t=l.trilla;
  const D=calcCostoTri(mesAnioTrillaDe(l),costos,lotes).costoTriKg;
  return cl&&t?.kg_excelso>0?Math.round((cl.total*pesoATrilladora(l))/t.kg_excelso)+Math.round(D):0;
};

export const calcCostoTriCF=(mesAnio,costos,lotesFino)=>{
  const costosTri=(costos||[]).filter(c=>c.centro==="Bodega Cafe Fino"&&c.mesAnio===mesAnio).reduce((s,c)=>s+c.valor,0);
  const kgEx=(lotesFino||[]).filter(l=>l.para_trilladora&&mesAnioTrillaDe(l)===mesAnio&&l.trilla?.kg_excelso>0).reduce((s,l)=>s+(l.trilla.kg_excelso||0),0);
  return{costosTri,kgEx,costoTriKg:kgEx>0?costosTri/kgEx:0};
};
...
export const costoKgExFinoDe=(grupo,costos,lotesFino)=>{
  ...
  if(base===0)return 0;
  const d=calcCostoTriCF(mesAnioTrillaDe(grupo[0]),costos,lotesFino).costoTriKg||0;
  return base+d;
};

export const calcCostoTuesteMes=(mesAnio,costos,historico)=>{
  const costosTueste=(costos||[]).filter(c=>c.centro==="Tostado"&&c.mesAnio===mesAnio).reduce((s,c)=>s+c.valor,0);
  const kgTostado=(historico||[]).filter(t=>mesAnioDe(t.fecha)===mesAnio&&(t.kg_cafe_tostado||0)>0).reduce((s,t)=>s+(t.kg_cafe_tostado||0),0);
  return{costosTueste,kgTostado,costoTuesteKg:kgTostado>0?costosTueste/kgTostado:0};
};

export const calcCostoMaquilaMes=(mesAnio,costos,maquilas)=>{
  const costosMQ=(costos||[]).filter(c=>c.centro==="Maquila"&&c.mesAnio===mesAnio).reduce((s,c)=>s+c.valor,0);
  const kgRecibidos=(maquilas||[]).filter(m=>(m.mesAnio||mesAnioDe(m.fecha))===mesAnio&&(m.kg_recibidos||0)>0).reduce((s,m)=>s+(m.kg_recibidos||0),0);
