...
export function DashboardTrilla({lotes,costos}){
  ...
  const triEntrada=lotesTF.reduce((s,l)=>s+pesoATrilladora(l),0);
  const triMerma=lotesTF.reduce((s,l)=>s+(l.trilla.kg_merma||0),0);
  const triPasillas=lotesTF.reduce((s,l)=>s+(l.trilla.kg_pasillas||0),0);
  const lotesConMerma=lotesTF.filter(l=>pesoATrilladora(l)>0);
  const pctMermaProm=lotesConMerma.length>0?lotesConMerma.reduce((s,l)=>s+((l.trilla.kg_merma||0)/pesoATrilladora(l)*100),0)/lotesConMerma.length:0;

  // Excelso producido por producto (con costo/kg incluyendo D = costo Trilladora prorrateado del mes real de cada lote)
  const porProd={};
  lotesTF.forEach(l=>{
    ...
    const costoProduccion=cl?cl.total*pesoATrilladora(l):0;
  ...
  // Factor Pretrilla Ponderado vs Factor Industrial Ponderado (ponderados por pesoATrilladora(l))
  // Excluye lotes "placeholder de carga masiva": factor_industrial===0 Y factor_pretrilla_ponderado===0
  ...
  // Tendencia semanal: semana calculada EN VIVO desde trilla.fecha_trilla (no existe un campo "semana" guardado
  // en trilla/salidas_trilladora — el unico campo "semana" del lote es de su ingreso a bodega, no de la trilla real)
  ...
  const tiles=[
    ...
    {label:"Costo/kg Excelso Trilladora",value:costoTriKgDash>0?fmtCOP(Math.round(costoTriKgDash)):"—",sub:"costo centro Trilladora / kg excelso",col:C.purple,icon:"⚙️",fs:15},
  ];
  ...
  return(<>
    ...
    <div style={{...S.card,marginBottom:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div><div style={{fontWeight:700,fontSize:14,color:C.navy}}>Excelso Producido por Producto</div><div style={{fontSize:11,color:C.textDim,marginTop:2}}>Costo/kg incluye costo Trilladora prorrateado (D) del mes real de cada lote</div></div>
