...
export function BlendFino({lotesFino,setLotesFino,blendsFino,setBlendsFino,setBlendsTostado,inventariosMensuales,setInventariosMensuales}){
  ...
  const mesesBF=ordenarMesAnio(blendsFino.map(b=>b.mesAnio));
  ...
  const blendsFiltrados=blendsFino.filter(b=>{
    if(filMesBF&&b.mesAnio!==filMesBF)return false;
  ...
  return(<div>
    ...
    {tab==="inventario"&&(<div style={S.card}>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",marginBottom:12}}>
        ...
        <select style={{...S.select,flex:"1 1 130px",maxWidth:160}} value={filMesBF} onChange={e=>setFilMesBF(e.target.value)}><option value="">Todos los meses</option>{mesesBF.map(m=>(<option key={m} value={m}>{formatMesAnio(m)}</option>))}</select>
