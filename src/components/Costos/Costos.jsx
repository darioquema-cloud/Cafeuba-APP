...
export function Costos({costos,setCostos}){
  const [modal,setModal]=useState(false);const [editId,setEditId]=useState(null);const [form,setForm]=useState({fecha:today(),mes:MESES[new Date().getMonth()],mesAnio:mesAnioDe(today()),tipo:TIPOS_COSTO[0],descripcion:"",valor:"",centro:CENTROS[0]});const [fil,setFil]=useState("todos");
  ...
  const blankFormC=()=>({fecha:today(),mes:MESES[new Date().getMonth()],mesAnio:mesAnioDe(today()),tipo:TIPOS_COSTO[0],descripcion:"",valor:"",centro:CENTROS[0]});
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:20}}><KPI label="Total Costos" value={fmtCOP(costos.reduce((s,c)=>s+c.valor,0))} col={C.red} icon="🧾" autoFit/><KPI label="Central Beneficio" value={fmtCOP(porC["Central de Beneficio"]||0)} col={C.teal} icon="🏭" autoFit/><KPI label="Trilladora" value={fmtCOP(porC["Trilladora"]||0)} col={C.purple} icon="⚙️" autoFit/><KPI label="Tostado" value={fmtCOP(porC["Tostado"]||0)} col={C.orange} icon="☕" autoFit/><KPI label="Maquila" value={fmtCOP(porC["Maquila"]||0)} col={C.gold} icon="🔄" autoFit/><KPI label="Bodega Cafe Fino" value={fmtCOP(porC["Bodega Cafe Fino"]||0)} col={C.green} icon="📦" autoFit/><KPI label="Centro de Costos Verde" value={fmtCOP(porC["Centro de Costos Verde"]||0)} col={C.navy} icon="🌱" autoFit/><KPI label="Registros" value={costos.length} col={C.accent} icon="📋" autoFit/></div>
    ...
    {modal&&(<Modal title={editId?"Editar Costo":"Registrar Nuevo Costo"} onClose={()=>{setModal(false);setEditId(null);}}>
      <div style={{display:"flex",flexWrap:"wrap",gap:"0 12px"}}>
        <Fld label="Fecha" half><input style={S.input} type="date" value={form.fecha} onChange={e=>{const d=e.target.value;setForm(p=>({...p,fecha:d,mes:mesDe(d)||p.mes,mesAnio:mesAnioDe(d)||p.mesAnio}));}}/></Fld>
