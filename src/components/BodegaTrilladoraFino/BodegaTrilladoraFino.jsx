...
import{mesDe,mesAnioDe,mesAnioTrillaDe,ordenarMesAnio,formatMesAnio}from"../../lib/dates";
...
export function BodegaTrilladoraFino({lotesFino,setLotesFino,setBlendsTostado,costos,inventariosMensuales,setInventariosMensuales}){
  ...
  const mesesBTF=ordenarMesAnio(trilledFino.map(l=>mesAnioTrillaDe(l)));
  ...
  // La entidad del arqueo es el GRUPO (corte/trillado), igual que en Bodega Trilladora normal.
  // Al cerrar se genera una salida sintetica en salidas_trilladora del lote REPRESENTANTE del
  // grupo (destino_key:"ajuste_inventario"), misma logica de signo: peso_salida=-diferencia_kg.
  ...
  const inventariosBTF=(inventariosMensuales||[]).filter(i=>i.modulo==="trilladora_cf");
  const invActivo=inventariosBTF.find(i=>i.id===selInvId)||null;
  ...
  const crearInventario=()=>{
    ...
    const detalle=gruposActivos.map(grupo=>{
      ...
      return {
        ...
        stock_teorico:stockGrupoBTF(grupo),
        stock_fisico:null,
        diferencia_kg:0,
        diferencia_pct:0,
  ...
  const cerrarInventario=(inv)=>{
    ...
    if(!window.confirm("¿Cerrar este inventario? Se generara un ajuste de stock en Bodega Trilladora Fino para cada grupo con diferencia, y ya no se podra editar salvo que lo reabras."))return;
  ...
  const exportarPlanillaExcel=(inv)=>{
    const data=inv.detalle.map(d=>({"Corte/Trillado":d.lote_codigo,"Producto":d.producto||"","Stock Teorico (kg)":+d.stock_teorico.toFixed(1),"Stock Fisico (kg)":""}));
    const wb=XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(data),"Planilla Conteo");
    XLSX.writeFile(wb,"Planilla-Inventario-TrilladoraCF-"+(inv.mes||"")+"-"+inv.anio+".xlsx");
  };
  // Exporta el Acta de Inventario (PDF) una vez cerrado. Valor unitario = costoKgExFinoDe(grupo),
  // el mismo Costo/kg Ex que ya muestra la tabla principal de Bodega Trilladora Fino.
  const exportarActaPDF=(inv)=>{
    ...
    doc.text("CafeUba — Bodega Trilladora Fino",14,18);
    ...
    if(significativos.length===0){
      ...
      autoTable(doc,{
        startY:y+4,
        head:[["Corte/Trillado","Producto","Teorico kg","Fisico kg","Dif. kg","Dif. %","Nota"]],
    ...
    doc.save("Acta-Inventario-TrilladoraCF-"+(inv.mes||"")+"-"+inv.anio+".pdf");
  };
  ...
  const gruposFiltrados=todosGrupos.filter(grupo=>{
    if(filtroMes&&mesAnioTrillaDe(grupo[0])!==filtroMes)return false;
  ...
  const totalSalidasTodasBTF=trilledFino.reduce((s,l)=>s+(l.salidas_trilladora||[]).reduce((a,b)=>a+b.peso_salida,0),0);
  const stockTotalBTF=totalExcelsoBTF-totalSalidasTodasBTF;
  ...
  const eliminarSalidaBTF=(loteId,salidaId)=>{
    if(!window.confirm("¿Eliminar esta salida? Esta acción no se puede deshacer."))return;
    setLotesFino(p=>p.map(l=>l.id===loteId?{...l,salidas_trilladora:(l.salidas_trilladora||[]).filter(s=>s.id!==salidaId)}:l));
  };
  const regSalidaBTF=()=>{
  ...
  return(<div>
    ...
    {tab==="inventario"&&(<>
      <div style={{...S.card,display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <select style={{...S.select,width:150}} value={filtroMes} onChange={e=>setFiltroMes(e.target.value)}><option value="">Todos los meses</option>{mesesBTF.map(m=>(<option key={m} value={m}>{formatMesAnio(m)}</option>))}</select>
      ...
      {(filtroMes||filtroProducto||busqueda)&&(()=>{
        ...
        const sumSalTodas=gruposFiltrados.reduce((s,g)=>s+g.reduce((a,x)=>a+(x.salidas_trilladora||[]).reduce((b,c)=>b+c.peso_salida,0),0),0);
        // Excluye "ajuste_inventario" de las salidas "reales" — el ajuste corrige el stock pero
        ...
        return(<div style={{background:C.navy,borderRadius:8,padding:"10px 16px",marginBottom:14,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:8}}>
          ...
          <div style={{textAlign:"center"}}><div style={{color:"rgba(255,255,255,0.6)",fontSize:9,fontWeight:700,letterSpacing:1}}>KG STOCK</div><div style={{color:"#6ee7b7",fontWeight:700,fontSize:15}}>{fmt(sumExc-sumSalTodas)} kg</div></div>
    ...
    {tab==="inventario_mensual"&&(<>
      {!invActivo?(<>
        <div style={{...S.card,display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
          <div><div style={{fontWeight:700,fontSize:14,color:C.navy}}>Inventario Mensual — Bodega Trilladora Fino</div><div style={{fontSize:11,color:C.textDim,marginTop:2}}>Arqueo de existencias: compara el stock teorico del sistema (por corte) contra el conteo fisico</div></div>
        ...
        {inventariosBTF.length===0?(
          ...
          <div style={S.card}>
            ...
            <tbody>{[...inventariosBTF].sort((a,b)=>(b.fecha_conteo||"").localeCompare(a.fecha_conteo||"")).map(inv=>{
        ...
        return(<>
          ...
          <div style={{...S.card,display:"flex",gap:10,alignItems:"center",marginBottom:12}}>
            <input style={{...S.input,flex:1,minWidth:180}} placeholder="Buscar por corte/trillado..." value={busquedaInv} onChange={e=>setBusquedaInv(e.target.value)}/>
            <span style={{color:C.textFaint,fontSize:12,whiteSpace:"nowrap"}}>{detalleFiltrado.length} de {detalleView.length} grupos</span>
          </div>
          <div style={S.card}>
            <TablaScrollV><table style={{width:"100%",borderCollapse:"collapse",minWidth:1000}}><thead><tr>
              {["Corte/Trillado","Producto","Stock Teorico kg","Stock Fisico kg","Diferencia kg","Diferencia %","Estado","Nota Justificacion"].map(h=>(<th key={h} style={S.th}>{h}</th>))}
            </tr></thead>
            <tbody>{detalleFiltrado.map(d=>(
              <tr key={d.grupo_repr_id}>
                <td style={{...S.td,fontWeight:700,color:C.green,fontFamily:"monospace"}}>{d.lote_codigo}</td>
    ...
    {modalSalida&&selGrupo&&(<Modal title={(editSalidaId?"Editar Salida - ":"Registrar Salida - ")+(selGrupo[0].trilla?.nombre_trillado||selGrupo[0].codigo)} onClose={()=>{setModalSalida(false);setEditSalidaId(null);setErrSalida("");}}>
      ...
      {formSalida.destino_key==="uba_tostado"&&(<div style={{background:C.orangeBg,border:"1px solid "+C.orange+"30",borderRadius:6,padding:"8px 12px",fontSize:12,color:C.orange,fontWeight:600,marginBottom:10}}>&#8505; Este lote quedara disponible en UBA Tostado para tostarse en uno o varios batches.</div>)}
