...
import{semanaISO,mesDe,mesAnioDe,mesTrillaDe,mesAnioTrillaDe,ordenarMesAnio,formatMesAnio}from"../../lib/dates";
...
export function BodegaTrilladora({lotes,setLotes,costos,setLotesFino,inventariosMensuales,setInventariosMensuales}){
  ...
  const mesesT=ordenarMesAnio(trilledLotes.map(l=>mesAnioTrillaDe(l)));
  ...
  const gruposTFiltrados=construirGruposT(trilledLotes).filter(grupo=>{
    if(filtroMes&&!grupo.some(l=>mesAnioTrillaDe(l)===filtroMes))return false;
  ...
  const abrirSalidaT=(l,costoGrupo)=>{
    const stock=stockGrupoDe(l);
    if(stock<=0)return;
    setSelLoteT(l);
    setEditSalidaTId(null);
    setFormSalidaT({fecha:today(),factura:"",remision:"",cliente:"",destino_key:"",peso_salida:"",valor_kg:costoGrupo||costoKgExDe(l)||"",valor_total:"",observaciones:""});
    setErrSalidaT("");
  ...
  const crearInventario=()=>{
    ...
    const nuevo={id:genId(),modulo:"trilladora",mes:mesDe(formNuevoInv.fecha_conteo),mesAnio:mesAnioDe(formNuevoInv.fecha_conteo),anio:new Date(formNuevoInv.fecha_conteo+"T00:00:00").getFullYear(),seccion:"trilladora",fecha_conteo:formNuevoInv.fecha_conteo,usuario_conteo:formNuevoInv.usuario_conteo.trim(),estado:"borrador",detalle};
  ...
  return(<div>
    ...
    {tab==="inventario"&&(<><div style={{...S.card,display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
      ...
      <select style={{...S.select,width:150}} value={filtroMes} onChange={e=>setFiltroMes(e.target.value)}><option value="">Todos los meses</option>{mesesT.map(m=>(<option key={m} value={m}>{formatMesAnio(m)}</option>))}</select>
    ...
    {(filtroMes||filtroProducto||busqueda)&&(()=>{
      ...
      const sumTValStk=gruposTFiltrados.reduce((s,g)=>{
        ...
        const D=calcCostoTri(mesAnioTrillaDe(g[0]),costos,lotes).costoTriKg;
    ...
    {modalSalidaT&&selLoteT&&(<Modal title={(editSalidaTId?"Editar Salida Trilladora - ":"Registrar Salida Trilladora - ")+selLoteT.codigo} onClose={()=>{setModalSalidaT(false);setEditSalidaTId(null);setErrSalidaT("");}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:"0 12px"}}>
        ...
        <Fld label="Precio por Unidad (kg COP)" half><input style={{...S.input,...(formSalidaT.destino_key==="bodega_cf"?{background:C.panel2,color:C.textDim,cursor:"not-allowed"}:{})}} type="number" readOnly={formSalidaT.destino_key==="bodega_cf"} value={formSalidaT.valor_kg} onChange={e=>setFormSalidaT(p=>({...p,valor_kg:e.target.value,valor_total:+e.target.value*(+formSalidaT.peso_salida||0)||""}))}/></Fld>
        ...
        <Fld label="Cliente / Destino"><SelectDestino value={formSalidaT.cliente} destinoKey={formSalidaT.destino_key} onChange={(v,k)=>setFormSalidaT(p=>{
          if(k==="bodega_cf"){const vkg=costoKgExDe(selLoteT)||"";return{...p,cliente:v,destino_key:k,valor_kg:vkg,valor_total:vkg&&p.peso_salida?+vkg*(+p.peso_salida||0):p.valor_total};}
