...
export function BodegaFino({lotesFino,setLotesFino,setBlendsFino,setBlendsTostado,lotes,inventariosMensuales,setInventariosMensuales}){
  ...
  const guardarPre=()=>{
    if(!selLote)return;
    setLotesFino(p=>p.map(l=>l.id===selLote.id?{...l,pretrilla:{
  ...
  const reg=()=>{
    ...
    if(editId){
      ...
      setLotesFino(p=>[{id:genId(),codigo:genCod(),fecha:form.fecha,mes:mesDe(form.fecha),mesAnio:mesAnioDe(form.fecha),semana:semanaISO(form.fecha),producto:form.producto,tipo_ingreso:form.tipo_ingreso,proveedor:form.proveedor,kg_producto:+form.kg_producto,costo_compra_kg:+form.costo_compra_kg,notas:form.notas,salidas_bodega:[],trilla:null,salidas_trilladora:[]},...p]);
    }
    setModal(false);
  };
  ...
  const eliminarSalida=(loteId,salidaId)=>{
    if(!window.confirm("¿Eliminar esta salida? Esta acción no se puede deshacer."))return;
    setLotesFino(p=>p.map(l=>l.id===loteId?{...l,salidas_bodega:(l.salidas_bodega||[]).filter(s=>s.id!==salidaId)}:l));
  };
  const regSalida=()=>{
    ...
    if(esTrilladoraFino){
      setLotesFino(p=>[{id:genId(),codigo:selLote.codigo,fecha:formSalida.fecha,mes:mesDe(formSalida.fecha),mesAnio:mesAnioDe(formSalida.fecha),semana:semanaISO(formSalida.fecha),producto:selLote.producto||"",proveedor:"Bodega Café Fino",kg_producto:peso,costo_compra_kg:vkg||selLote.costo_compra_kg||0,notas:"Trasladado desde Bodega CF — "+selLote.codigo,salidas_bodega:[],trilla:null,salidas_trilladora:[],pretrilla:selLote?.pretrilla||null,trazabilidad:{codigo_lote_origen:selLote?.codigo||"",fecha_proceso:"",fecha_trilla:"",fecha_secado:"",lotes_blend:[]},para_trilladora:true},...p]);
    }
  ...
  const crearInventario=()=>{
    ...
    const nuevo={id:genId(),modulo:"bodega_cf",mes:mesDe(formNuevoInv.fecha_conteo),mesAnio:mesAnioDe(formNuevoInv.fecha_conteo),anio:new Date(formNuevoInv.fecha_conteo+"T00:00:00").getFullYear(),seccion:"bodega_cf",fecha_conteo:formNuevoInv.fecha_conteo,usuario_conteo:formNuevoInv.usuario_conteo.trim(),estado:"borrador",detalle};
  ...
  const cerrarInventario=(inv)=>{
    ...
    setLotesFino(p=>p.map(l=>{
      // Quita cualquier ajuste anterior de ESTE MISMO cierre (misma factura), para que
      // reabrir+cerrar reemplace en vez de acumular ajustes duplicados.
      const salidasSinAjusteViejo=(l.salidas_bodega||[]).filter(s=>!(s.destino_key==="ajuste_inventario"&&s.factura===factura));
      if(!(l.id in ajustesPorLote))return{...l,salidas_bodega:salidasSinAjusteViejo};
      ...
      return{...l,salidas_bodega:[...salidasSinAjusteViejo,nuevaSalida]};
    }));
  ...
  return(<div>
    ...
    {tab==="inventario"&&(<div style={S.card}>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",marginBottom:12}}>
        ...
        <select style={{...S.select,flex:"1 1 130px",maxWidth:160}} value={filMesB} onChange={e=>setFilMesB(e.target.value)}><option value="">Todos los meses</option>{mesesB.map(m=>(<option key={m} value={m}>{formatMesAnio(m)}</option>))}</select>
    ...
    {modalPre&&selLote&&(<Modal title={"Pre-Trilla - "+selLote.codigo} onClose={()=>setModalPre(false)}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:14,marginBottom:14}}>
        ...
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          <tbody>
            {[["Negro (gr)","negro"],["Vinagre (gr)","vinagre"],["Grano Verde (gr)","grano_verde"],["Reventado (gr)","reventado"],["Dano de Maquina (gr)","dano_maquina"],["Broca Severa (gr)","broca_severa"],["Broca de Punto (gr)","broca_punto"]].map(([label,key])=>(
              <tr key={key}>
                <td style={{border:"1px solid "+C.border,padding:"6px 10px",fontSize:12,color:C.text}}>{label}</td>
                <td style={{border:"1px solid "+C.border,padding:2}}>
                  <input style={{...S.input,border:"none",textAlign:"right"}} type="number" value={formPre[key]} onChange={e=>setFormPre(p=>({...p,[key]:e.target.value}))}/>
